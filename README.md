...
## Drafts in the Mac app

Use **Create New Draft** beside the draft dropdown to copy the selected draft and begin working in the copy. Existing scripts start as **Draft 1**; new copies receive the next available draft number. **Rename** gives a draft a meaningful name such as “Alternate ending.” Names must be nonempty and unique within the workspace.

Every draft is independently editable and contains its screenplay, title page, scene synopses and structure labels, beat board, and Writers’ Room conversations and treatment. Switching drafts captures the latest edits first and updates the native document immediately. Undo starts fresh when switching drafts, so it cannot undo another draft’s work; returning to a draft restores its saved content. Model responses from a view you have left cannot update another draft.

All drafts and the selected draft are saved in the same local **.pica** workspace and restored when you reopen it. Exports contain only the selected draft, with its draft name in the suggested filename; the screenplay title page is unchanged. Multi-draft comparison and deletion are not included yet.

Workspaces saved by this version use format version 2. Older single-draft files still open; older Pica builds reject version 2 rather than accidentally discard drafts. Use FDX, Fountain, or PDF export to share a selected draft with another application.
...
The **Outline** tab shows the same scenes as index cards: heading, synopsis, cast, page and length, a colour, and structure labels (Act One, Midpoint…) between cards. Drag a card to reorder the script, add or delete scenes, and open any scene in the editor. The **Beat Board** is a freeform canvas of ideas: double-click to add a beat, use the visible **Drag to move** handle (or arrow keys while it is focused) to arrange cards, and send a beat into the script as a new scene with the beat's text as its synopsis. Scene links collapse into a summary; expand one to jump to its scenes. **Tidy board** arranges cards into rows using their measured heights and the available width, without changing their text. Existing layouts stay in place until you move or tidy them.

In the Writers' Room, choose **Sequence overview**, **Detailed dramatic turns** (the default), or **Scene by scene** before laying out beats. There is no beat-count target or per-reply card quota. **Analyze existing script** grounds cards in the supplied screenplay; **Develop the story** allows proposed additions, marked as not yet in the script. These choices are saved with the workspace. Model response limits still apply: an unfinished block is not applied and is reported, and a complete partial response should explain its remaining scope so you can ask the room to continue. Existing cards are retained when you request finer detail.
