...
describe('layout basics', () => {
  it('separates elements with a blank row except inside a dialogue block', () => {
    const { pages } = layoutElements([sh(), a('Bob enters.'), ch(), p(), d('Hi.'), a('He leaves.')]);
  ...
  it('fills a page to 55 rows and starts the next without a leading blank', () => {
    const { pages, breaks } = layoutElements([a(actionOf(30)), a(actionOf(24)), a('tail')]);
  ...
  it('honours manual page breaks', () => {
    const { pages, breaks } = layoutElements([a('one'), pb(), a('two')]);
...
describe('action splitting', () => {
  it('splits a long action across pages at a line boundary', () => {
    const { pages, breaks } = layoutElements([a(actionOf(50)), a(actionOf(10))]);
  ...
  it('never leaves a single line of action at the bottom or top of a page', () => {
    const bottom = layoutElements([a(actionOf(52)), a(actionOf(10))]);
    ...
    const single = layoutElements([a(actionOf(53)), a(actionOf(10))]);
    expect(single.breaks[0]).toMatchObject({ elementIndex: 1, lineIndex: 0 });
    const top = layoutElements([a(actionOf(50)), a(actionOf(5))]);
    // 4 available; 5 - 4 = 1 would be carried over -> move whole element.
    expect(top.breaks[0]).toMatchObject({ elementIndex: 1, lineIndex: 0 });
  });
});

describe('scene headings', () => {
  it('moves a heading to the next page when fewer than two lines of its scene would follow', () => {
    const { breaks, elements } = layoutElements([a(actionOf(52)), sh(), a(actionOf(5))]);
    expect(breaks[0]).toMatchObject({ elementIndex: 1, lineIndex: 0 });
    expect(elements[1].page).toBe(2);
  });

  it('keeps a heading when two lines of the following action fit', () => {
    const { breaks } = layoutElements([a(actionOf(49)), sh(), a(actionOf(5))]);
...
describe('dialogue', () => {
  it('splits long dialogue with (MORE) and a (CONT\'D) cue', () => {
    const { pages, breaks } = layoutElements([a(actionOf(45)), ch('ALICE'), d(dialogueOf(12))]);
  ...
  it('does not double up an existing (CONT\'D) and keeps extensions', () => {
    const { breaks } = layoutElements([a(actionOf(45)), ch("ALICE (V.O.) (CONT'D)"), d(dialogueOf(12))]);
    expect(breaks[0].contdCue).toBe("ALICE (V.O.) (CONT'D)");
  });

  it('never strands a character cue at the bottom of a page', () => {
    const { breaks, pages } = layoutElements([a(actionOf(52)), ch(), d(dialogueOf(4))]);
  ...
  it('dialogue that exactly fills the page is not split', () => {
    const { breaks, pages } = layoutElements([a(actionOf(45)), ch(), d(dialogueOf(8))]);
  ...
  it('carries at least two lines when it does split', () => {
    const { breaks } = layoutElements([a(actionOf(45)), ch(), d(dialogueOf(9))]);
  ...
  it('breaks before a parenthetical rather than splitting it', () => {
    const { breaks, pages } = layoutElements([a(actionOf(45)), ch(), d(dialogueOf(7)), p('(a long parenthetical here)'), d(dialogueOf(3))]);
  ...
  it('keeps a cue with its parenthetical and two lines of dialogue', () => {
    const { breaks } = layoutElements([a(actionOf(50)), ch(), p(), d(dialogueOf(4))]);
    ...
    const { pages } = layoutElements([a(actionOf(50)), ch(), p(), d(dialogueOf(4))]);
    expect(pages[0].rows.length).toBe(50);
  });

  it('splits dialogue longer than a page repeatedly with CONT\'D each time', () => {
    const { pages, breaks } = layoutElements([ch('LONG'), d(dialogueOf(120))]);
...
describe('transitions', () => {
  it('pulls a short preceding element down so a transition never opens a page', () => {
    const { pages, breaks } = layoutElements([a(actionOf(51)), a('Short.'), t()]);
    // 51 + blank + Short. = 53; transition needs blank + 1 = 55 -> fits. Use 52 instead:
    const tight = layoutElements([a(actionOf(52)), a('Short.'), t()]);
...
describe('pageAt', () => {
  it('maps an element offset to its page, including inside a split element', () => {
    const layout = layoutElements([a(actionOf(50)), a(actionOf(10))]);
